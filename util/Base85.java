...
public class Base85 {
    ...
    /**
     * This is a base class for encoding data using the Base85 encoding scheme, in the same style as Base64 encoder.
     * Just override {@link #getEncodeMap()} to create a fully functional encoder. Encoder instances can be safely
    ...
    public static abstract class Encoder {
         ...
         */
        public final int encode(final byte[] data, final int offset, final int length, final byte[] out,
        ...
        /**
         * Encode the data as one block in reverse output order. This is the strict algorithm specified by RFC 1924 for
         * IP address encoding, when the data is exactly 16 bytes (128 bits) long. Because the whole input data is
         * encoded as one big block, this is much less efficient than the more common encodings.
         *
        ...
        /**
         * Encode part of data as one block in reverse output order into output array. This is the strict algorithm
         * specified by RFC 1924 for IP address encoding, when the data part is exactly 16 bytes (128 bits) long.
         * Because the whole input data part is encoded as one big block, this is much less efficient than the more
         * common encodings.
         *
        ...
        protected int _encodeDangling(final byte[] encodeMap, final byte[] out, final int wi, final ByteBuffer buffer,
                int leftover) {
    ...
    /**
     * This class encodes data in the Base85 encoding scheme using the character set described by IETF RFC 1924, but in
     * the efficient algorithm of Ascii85 and Z85. This scheme does not use quotes, comma, or slash, and can usually be
     * used in sql, json, csv etc. without escaping.
     * <p>
    ...
    public static class Rfc1924Encoder extends Encoder {
        private static final byte[] ENCODE_MAP = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!#$%&()*+-;<=>?@^_`{|}~".getBytes(
                US_ASCII);
    ...
    /**
     * This class encodes data in the Base85 encoding scheme Z85 as described by ZeroMQ. This scheme does not use quotes
     * or comma, and can usually be used in sql, json, csv etc. without escaping.
     * <p>
     * Encoder instances can be safely shared by multiple threads.
     *
    ...
    public static class Z85Encoder extends Encoder {
        private static final byte[] ENCODE_MAP = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ.-:+=^!/*?&<>()[]{}@%$#".getBytes(
                US_ASCII);
    ...
    /**
     * This class encodes data in the Ascii85 encoding (Adobe variant without &lt;~ and ~&gt;). Supports "z" and "y"
     * compression, which can be disabled individually. Line break is not supported.
     * <p>
     * Encoder instances can be safely shared by multiple threads.
     *
    ...
    public static class Ascii85Encoder extends Encoder {
        private static final byte[] ENCODE_MAP = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstu".getBytes(
                US_ASCII);
    ...
    /**
     * This is a skeleton class for decoding data in the Base85 encoding scheme. in the same style as Base64 decoder.
     * Just override {@link #getDecodeMap()} and {@link #getName()} to create a fully functional decoder. Decoder
     * instances can be safely shared by multiple threads.
     */
    public static abstract class Decoder {
        /**
         * Calculate byte length of decoded data. Assumes data is correct; use test method to validate data.
         *
         ...
         * @return decoded binary data
         *
         * @throws IllegalArgumentException if offset or length is negative, or if data array is not big enough (data
         *                                  won't be written)
         */
         ...
         *
         * @throws IllegalArgumentException if offset or length is negative, or if either array is not big enough (data
         *                                  won't be written)
         */
        public final int decode(final byte[] data, final int offset, final int length, final byte[] out,
                final int out_offset) {
        ...
        /**
         * Decode the data as one block in reverse input order. This is the strict algorithm specified by RFC 1924 for
         * IP address decoding, when the data is exactly 16 bytes (128 bits) long.
         *
         * @param data Byte data to encode
         *
         * @return Encoded data in ascii encoding
        ...
        /**
         * Decode part of data as one block in reverse input order into output array. This is the strict algorithm
         * specified by RFC 1924 for IP address decoding, when the data part is exactly 16 bytes (128 bits) long.
         *
         * @param data       array to read data from
         * @param offset     byte offset to start reading data
         * @param length     number of byte to read
        ...
        protected int _decodeDangling(final byte[] decodeMap, final byte[] in, final int ri, final ByteBuffer buffer,
                int leftover) {
            if (leftover == 1) {
                throwMalformed(null);
            }
            long sum = decodeMap[in[ri]] * Power4 + decodeMap[in[ri + 1]] * Power3 + 85;
            if (leftover >= 3) {
        ...
        protected void _putData(ByteBuffer buffer, byte[] map, byte[] in, int ri) {
            buffer.putInt(0, (int) (map[in[ri]] * Power4 + map[in[ri + 1]] * Power3 + map[in[ri + 2]] * Power2 +
                                            map[in[ri + 3]] * 85 + map[in[ri + 4]]));
        }
    ...
    /**
     * This class decodes data in the Base85 encoding using the character set described by IETF RFC 1924, in the
     * efficient algorithm of Ascii85 and Z85. Malformed data may or may not throw IllegalArgumentException on decode;
     * call test(byte[]) to check data if necessary. Decoder instances can be safely shared by multiple threads.
     *
     * @see <a href="https://tools.ietf.org/html/rfc1924">RFC 1924</a>
     */
    ...
    /**
     * This class decodes data in the Base85 encoding scheme Z85 as described by ZeroMQ. Malformed data may or may not
     * throw IllegalArgumentException on decode; call test(byte[]) to check data if necessary. Decoder instances can be
     * safely shared by multiple threads.
     *
     * @see <a href="https://rfc.zeromq.org/spec:32/Z85/">Z85</a>
     */
    ...
    /**
     * This class decodes Ascii85 encoded data (Adobe variant without &lt;~ and ~&gt;). 'y' and 'z' are always
     * processed.  This keep the decoder simple. Malformed data may or may not throw IllegalArgumentException on decode;
     * call test(byte[]) to check data if necessary. Decoder instances can be safely shared by multiple threads.
     *
     * @see <a href="https://en.wikipedia.org/wiki/Ascii85">Ascii85</a>
     */
