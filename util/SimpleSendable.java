...
public interface SimpleSendable extends Sendable {
    ...
    default void simpleSendableInit(SendableBuilder builder) {
        ...
        for (final var method : childClass.getDeclaredMethods()) {
            if (method.isAnnotationPresent(SendableGetter.class)) entries.addGetter(this, method);
            if (method.isAnnotationPresent(SendableSetter.class)) entries.addSetter(this, method);
        }
        ...
        for (final var field : childClass.getDeclaredFields()) {
            if (field.isAnnotationPresent(SendableProperty.class)) entries.addProperty(this, field);
        }
    ...
    final class Entries {
        ...
        boolean exists(String name) {
            return intEntries.containsKey(name) || boolEntries.containsKey(name) || floatEntries.containsKey(name) || doubleEntries.containsKey(name) || stringEntries.containsKey(name);
        }

        void add(String name, BooleanSupplier getter) throws NullPointerException {
            this.boolEntries.put(name, new Tuple2<>(getter, this.exists(name) ? this.boolEntries.get(name).x2() : null));
        }

        void add(String name, BooleanConsumer setter) throws NullPointerException {
            this.boolEntries.put(name, new Tuple2<>(this.exists(name) ? this.boolEntries.get(name).x1() : null, setter));
        }
        ...
        void add(String name, FloatSupplier getter) throws NullPointerException {
            this.floatEntries.put(name, new Tuple2<>(getter, this.exists(name) ? this.floatEntries.get(name).x2() : null));
        }

        void add(String name, FloatConsumer setter) throws NullPointerException {
            this.floatEntries.put(name, new Tuple2<>(this.exists(name) ? this.floatEntries.get(name).x1() : null, setter));
        }

        void add(String name, DoubleSupplier getter) throws NullPointerException {
            this.doubleEntries.put(name, new Tuple2<>(getter, this.exists(name) ? this.doubleEntries.get(name).x2() : null));
        }

        void add(String name, DoubleConsumer setter) throws NullPointerException {
            this.doubleEntries.put(name, new Tuple2<>(this.exists(name) ? this.doubleEntries.get(name).x1() : null, setter));
        }

        void add(String name, Supplier<String> getter) throws NullPointerException {
            this.stringEntries.put(name, new Tuple2<>(getter, this.exists(name) ? this.stringEntries.get(name).x2() : null));
        }

        void add(String name, Consumer<String> setter) throws NullPointerException {
            this.stringEntries.put(name, new Tuple2<>(this.exists(name) ? this.stringEntries.get(name).x1() : null, setter));
        }
        ...
        void addSetter(Object obj, Method method) {
            ...
            if (NarrowType.isBool(takeType)) {
                ...
                if (takeType.isAssignableFrom(String.class)) {
                    ...
                    throw new UnsupportedOperationException("Type " + takeType.getName() + " is neither assignable nor constructible from string");
                }
            ...
            if (propertyInfo.access() != SendableProperty.AccessType.Out) {
                ...
                if (NarrowType.isBool(fieldType)) {
                    ...
                    if (fieldType.isAssignableFrom(String.class)) {
                        ...
                        throw new UnsupportedOperationException("Type " + fieldType.getName() + " is neither assignable nor constructible from string");
                    }
                    this.add(name, setter);
                }
            }
